...
namespace SocialsNetwork.DTO.Class
{
    ...
    public record UserResponseString(string Id, string UserName, string Email, string avatarURL, string Name);
    public record UserResponseBlock(Guid Id, string UserId, string ClaimValue, string Email, string AvatarURL, string BlockedId);
    public record NewUser(string Email, string Password, string Name, DateTime BirthDate, Genre Genre, string PhoneNumber, string Username);
